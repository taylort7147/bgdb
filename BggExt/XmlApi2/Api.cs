    ...
    public static class Api
    {
        ...
        private static ApiResult _ProcessApiCall(XElement? xml, string? rootElement = null)
        {
            ...
            else if (xml.Name == "errors")
            {
                ...
                {
                    result.Errors.Add(element.Value);
                }
            }
        ...
        public static async Task<ApiResult> GetBoardGame(int id)
        {
            ...
            if (result.Status == ApiResult.OperationStatus.Error)
            {
                return result;
            }
        ...
        */
        ...
        {
            var result = _ProcessApiCall(await _CallApi($"/xmlapi2/collection?username={userId}&subtype=boardgame&brief=1"), "items");
            if (result.Status == ApiResult.OperationStatus.Success)
            {
